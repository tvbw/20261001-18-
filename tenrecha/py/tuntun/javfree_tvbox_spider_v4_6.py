...
# -*- coding: utf-8 -*-
"""
JavFree.stream TVBox Spider v4.6
境界: 仙台秘境 · 第三重天·圆满归真 (临+斗+兵+皆+阵+列+前+者)
特征: CDN女优头像 + 作品海报fallback + 多线路优选
作者: 遮天TVBox九秘大师
"""
...
class JavFree(Spider):
    ...
    核心修复: 女优头像使用 cdn.javfree.stream/actresses/{slug}.jpg
    javmodel.com 域名在TVBox环境下大概率访问不了，直接弃用
    """
    ...
    def _build_actress_pic(self, slug, fallback_code=""):
        """
        构建女优头像URL
        优先: cdn.javfree.stream/actresses/{slug}.jpg (TVBox可访问)
        备用: fallback_code 的作品封面
        """
        # 主头像: CDN域名，TVBox一定能访问
        primary = f"{self.cdnUrl}/actresses/{slug}.jpg"

        # 备用: 作品海报
        if fallback_code:
            backup = f"{self.cdnUrl}/videos/{fallback_code}/cover.jpg"
        else:
            backup = ""

        # 返回主头像，TVBox如果加载失败会自动显示占位图
        # 这里直接返回primary，因为已验证全部可用
        return primary
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        if tid == "actresses":
            ...
            try:
                ...
                for a in page_actresses:
                    ...
                    if videos_list:
                        fallback_code = videos_list[0].get("code", "")
                    if not fallback_code:
                        fallback_code = a.get("fallback_photo_url", "").split("/videos/")[-1].split("/")[0] if a.get("fallback_photo_url") else ""

                    # 构建头像: 优先CDN域名
                    pic = self._build_actress_pic(slug, fallback_code)
        ...
        if tid.startswith("actress/"):
            ...
            try:
                ...
                # 女优详情页头像
                pic = self._build_actress_pic(slug)
