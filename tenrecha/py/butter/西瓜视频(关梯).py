...
HOST = 'https://71usplefawyj.se8131f9xg.cc'
...
def _http_get(u, headers):
    global _SESS, _CFFI
    # 1. use TVBox native instance (has correct TLS fingerprint on real device)
    if _SPIDER_INSTANCE is not None:
        try:
            r = _SPIDER_INSTANCE.fetch(u, headers=headers, timeout=8)
            _log('FETCH %d %s' % (getattr(r, 'status_code', 200), u[:50]))
            return r
        except Exception as e:
            _log('FETCH_ERR %s' % repr(e)[:60])
    # 2. try curl_cffi
    if _CFFI is None:
        try:
            ...
            _log('ENV no curl_cffi %s' % repr(e)[:60])
            _CFFI = False
    if _CFFI:
        for imp in ('chrome124', 'chrome120', 'chrome', 'safari17_0', 'safari'):
            try:
                r = _CFFI.get(u, headers=headers, impersonate=imp, timeout=8)
                _log('CFFI %s %d %s' % (imp, r.status_code, u[:50]))
                return r
            except Exception as e:
                _log('CFFI_ERR %s %s' % (imp, repr(e)[:50]))
    # 3. fallback to requests
    if _SESS is None:
        import requests as rq
        _SESS = rq.Session()
    r = _SESS.get(u, headers=headers, timeout=8, verify=False)
    _log('REQ %d %s' % (r.status_code, u[:50]))
    return r
...
def _proxy_headers(u):
    m = re.search(r'/([0-9a-f]{32})/', u)
    ref = HOST + '/poster.html?viewkey=' + m.group(1) if m else HOST + '/'
    return {
        'User-Agent': UA,
        'Referer': ref,
        'Accept': '*/*',
    ...
    if b[:4] == b'RIFF' and b[8:12] == b'WEBP':
        ...
        try:
            ...
            if _IMG_ENGINE == 3:
                ...
                import io
                out_buf = io.BytesIO()
                imageio.v2.imwrite(out_buf, imageio.v2.imread(io.BytesIO(b)), format='JPEG')
                return 'image/jpeg', out_buf.getvalue()
        except Exception:
            pass
        return 'image/webp', b
    if b[:2] == b'\xff\xd8':
    ...
    for attempt in range(2):
        try:
            with _SEM:
                ...
                r = _http_get(u, _proxy_headers(u))
                if r.status_code != 200:
                    _log('ERR status %d %s' % (r.status_code, u[:80]))
                    if attempt == 0:
                        continue
                    if '.m3u8' in u:
                        return r.status_code, 'application/vnd.apple.mpegurl', ('# Network error %d' % r.status_code).encode('utf-8')
                    return 200, 'image/gif', _PLACEHOLDER
                body = r.content
                if '.m3u8' in u:
                    m3u8_base = u.rsplit('/', 1)[0] + '/'
                    try:
                    ...
                    for line in m3u8_text.splitlines():
                        ...
                        if s.startswith('http'):
                            ...
                            seg_url = urljoin(m3u8_base, s)
                        lines.append('http://127.0.0.1:%d/71us?url=%s' % (_PORT, quote(seg_url, safe='')))
                    # rewrite any URI="..." inside # tags
                    rewritten = '\n'.join(lines)
                    for m_uri in list(re.finditer(r'URI="([^"]+)"', rewritten)):
                        old_uri = m_uri.group(1)
                        new_uri = 'http://127.0.0.1:%d/71us?url=%s' % (_PORT, quote(urljoin(m3u8_base, old_uri), safe=''))
                        rewritten = rewritten.replace('URI="%s"' % old_uri, 'URI="%s"' % new_uri)
                    _log('OK m3u8 %d lines %s' % (len(lines), u[:60]))
            ...
            if attempt == 0:
                continue
            if '.m3u8' in u:
                return 502, 'application/vnd.apple.mpegurl', ('# Network error: %s' % repr(e)[:80]).encode('utf-8')
            return 200, 'image/gif', _PLACEHOLDER
    return 200, 'image/gif', _PLACEHOLDER
...
class Spider(Spider):
    ...
    def playerContent(self, flag, id, vipFlags=None):
        ...
        _start_proxy()
        proxy_url = 'http://127.0.0.1:%d/71us?url=%s' % (_PORT, quote(id, safe=''))
